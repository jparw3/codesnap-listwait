...
export function Model({ email = 'jparw@jparw.com', showModel, setShowModel }: ModelProps) {
  return (
    <>
      <div
        className={clsx(
      ...
      >
        ...
        >
          <Circle type="tick" />
          <h2 className="font-normal text-3xl text-center text-white">We&apos;ve added you to our waiting list!</h2>
          <p className="text-xs text-white opacity-60 pt-3">We&apos;ll let you know when CodeSnap is ready.</p>
          <div className="bg-[rgb(38,38,44)] w-full border-t-[1px] border-[rgb(55,55,67)] rounded-xl p-4 mt-8 flex flex-col gap-4">
            <div className="h-10 p-[11px_12px_12px] bg-[rgb(20,20,26)] border-t-[1px] border-[rgb(7,7,13)] rounded-md flex items-center gap-[10px]">
              <svg width="24" height="24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M5.25 6.75h13.5v10.5H5.25V6.75Z" stroke="#fff" stroke-width="1.5"></path>
                <path d="M5.25 6.75 12 12l6.75-5.25" stroke="#fff" stroke-width="1.5"></path>
              </svg>
              <span className="text-sm text-white pb-px">{email}</span>
            </div>
            <p className="text-xs text-center text-white/50">CodeSnap is coming soon.</p>
            <p className="text-xs text-center text-white/50">
              Created by{' '}
              <Link target={'_blank'} className="underline hover:text-white transition" href="https://bento.me/jparw">
                Jparw
              </Link>{' '}
